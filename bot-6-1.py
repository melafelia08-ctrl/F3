...
OWNER_IGSID       = os.getenv("OWNER_IGSID", "")
...
def login() -> IGClient:
    """
    Session-only login — password sirf pehli baar.
    Priority:
      1. session.json  → cookie-restore, no password
      2. .env cookies  → inject + validate, no password
      3. interactive   → cookies manually enter karo, no password
    Har event SessionLog table mein + console mein record hota hai.
    """
    cl = IGClient()
    cl.delay_range = [1, 3]
    ...
    if sf.exists():
        try:
            ...
            # password-free validation — agar session valid hai toh kaam karega
            cl.get_timeline_feed()
            _log_session("RESTORED", cl, source="session_file")
            return cl
        except Exception as e:
            _log_session("EXPIRED", cl, source="session_file", detail=str(e))
            log.warning("session.json dead — naya login chahiye")
            sf.unlink(missing_ok=True)

    # ── 2. cookies source decide karo ────────────────────────────────────────
    if IG_SESSIONID:
        cookies = {
        ...
        source = "env"
        log.info("COOKIE SOURCE     | .env")
    else:
        cookies = _get_cookies_interactive()
        source  = "interactive"
        log.info("COOKIE SOURCE     | interactive")

    cl.set_settings({"cookies": cookies})

    # ── 3. password-free account validate ────────────────────────────────────
    try:
        info = cl.account_info()
        _log_session("COOKIE_LOGIN", cl, source=source,
                     detail=f"username={info.username}")

        # session.json save karo — agli baar password nahi mangega
        ...
        log.error("Cookie login fail: %s", e)
        raise RuntimeError(
            f"Cookie login fail hua: {e}\n"
            "sessionid expire ho gayi hogi — browser se fresh cookies lo."
        ) from e
